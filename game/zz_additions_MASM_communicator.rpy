    ...
    if additionIsEnabled("MASMC"):
        ...
        class MASM_DataCommunicator:
            ...
            def clientSend(toSend):
                if MASM_DataCommunicator.client is not None:
                    MASM_DataCommunicator.client.sendall(toSend).encode('utf-8')
        ...
        for dirpath,subdirs,files in os.walk('.'):
            if 'super_experimental_addition' in subdirs:
                enginePath = dirpath
                break
        ...
        if enginePath:
            if renpy.windows: # If we are running on Windows OS
                try:
                    engine = subprocess.Popen(renpy.loader.transfn(os.path.join(enginePath, '/super_experimental_addition/MASM.exe'))) # Open engine subprocess
                except:
                    pass
            elif renpy.linux or renpy.macintosh: # Linux / Mac..
                try:
                    engine = subprocess.Popen(renpy.loader.transfn(os.path.join(enginePath, '/super_experimental_addition/MASM.sh'))) # Good luck!
                except:
                    pass
